    ...
    <div className="flex items-center gap-1" aria-label={`${rating} star rating`}>
      {Array.from({ length: 5 }).map((_, index) => (
        <Star
          key={index}
          size={16}
          className={index < Number(rating) ? "fill-accent text-accent" : "text-muted-foreground/35"}
        />
      ))}
    </div>
  );
...
function ReviewCard({ review }) {
  return (
    <motion.article variants={fadeUp} className="rounded-3xl glass p-6 sm:p-7 h-full">
      ...
      <div className="mt-6 border-t border-border pt-5">
        <h3 className="font-display text-lg font-semibold">{review.client_name}</h3>
...
function ReviewForm({ onSubmitted }) {
  ...
  async function submitReview(event) {
    ...
    try {
      ...
      setNotice(data.message || "Thank you. Your review will appear after approval.");
      setForm(initialForm);
      onSubmitted?.();
    } catch (err) {
  ...
  return (
    ...
    >
      ...
      <label className="block">
        <span className="text-xs uppercase tracking-widest text-muted-foreground">Service taken</span>
        <input
          name="service"
          value={form.service}
          onChange={updateField}
      ...
      <label className="block">
        <span className="text-xs uppercase tracking-widest text-muted-foreground">Review *</span>
        <textarea
          name="message"
          value={form.message}
          onChange={updateField}
      ...
      {notice && <p className="rounded-xl bg-gradient-brand-soft border border-border p-4 text-sm">{notice}</p>}
...
export function ClientReviews({ showForm = false, limit = 6 }) {
  ...
  const visibleReviews = showForm ? reviews : reviews.slice(0, limit);
  ...
  return (
    <Section id="reviews" className={showForm ? "pt-0" : ""}>
      <motion.div variants={fadeUp} className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
        <div className="max-w-2xl">
          ...
          <h2 className="mt-4 font-display text-4xl sm:text-5xl font-semibold tracking-tight">
            What clients say about <span className="text-gradient">Trionyx</span>.
          </h2>
      ...
      {loading && <p className="mt-10 text-sm text-muted-foreground">Loading reviews...</p>}
      ...
      {!loading && !error && visibleReviews.length === 0 && (
        <div className="mt-10 rounded-3xl glass p-8 text-center">
          <p className="text-muted-foreground">
            Approved client reviews will appear here. Submit the first review from the reviews page.
          </p>
        </div>
      )}
      ...
      {showForm && (
        <div className="mt-16 grid lg:grid-cols-5 gap-8 items-start">
          <div className="lg:col-span-2 rounded-3xl bg-gradient-brand-soft border border-border p-8">
            <h3 className="font-display text-2xl font-semibold">Approval workflow</h3>
            <p className="mt-4 text-muted-foreground leading-relaxed">
              New reviews are saved as pending first. After you approve them in Supabase,
              they become visible to new clients on the website.
            </p>
          </div>
